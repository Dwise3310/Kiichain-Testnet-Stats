...
const LOG_CHUNK_SIZE = 2000;       // blocks per eth_getLogs call
const MAX_BLOCKS_PER_RUN = 100000; // safety cap so one invocation can't run forever
const TIME_BUDGET_MS = 45000;      // bail out early if we're close to timing out
const BALANCE_BATCH_SIZE = 10;     // concurrent balanceOf calls
...
async function getLogsChunked(provider, fromBlock, toBlock) {
  ...
  for (let start = fromBlock; start <= toBlock; start += LOG_CHUNK_SIZE) {
    ...
    try {
      ...
      // If the RPC rejects a range as too large, split it in half and retry once.
      if (end - start > 100) {
        const midpoint = Math.floor((start + end) / 2);
        const firstHalf = await getLogsChunked(provider, start, midpoint);
        const secondHalf = await getLogsChunked(provider, midpoint + 1, end);
        allLogs.push(...firstHalf, ...secondHalf);
      } else {
        console.error(`Failed to fetch logs for blocks ${start}-${end}:`, err.message);
      }
    }
  }
  return allLogs;
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Scan in smaller windows so we can respect the time budget and save
    // partial progress even if we don't reach `toBlock` this run.
    for (let start = fromBlock; start <= toBlock; start += LOG_CHUNK_SIZE * 5) {
      if (Date.now() - startedAt > TIME_BUDGET_MS) break;

      const windowEnd = Math.min(start + LOG_CHUNK_SIZE * 5 - 1, toBlock);
      const logs = await getLogsChunked(provider, start, windowEnd);
