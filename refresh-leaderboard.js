...
const LOG_CHUNK_SIZE = 5000;       // blocks per single eth_getLogs call
const CONCURRENCY = 6;             // parallel eth_getLogs calls per batch
const MAX_BLOCKS_PER_RUN = 5000000; // safety cap so one invocation can't run forever
const TIME_BUDGET_MS = 8000;       // conservative — stay well under Hobby plan's real function timeout
const REQUEST_TIMEOUT_MS = 6000;   // kill any single stuck RPC call rather than let it hang the batch
...
// Binary-search for the exact block the ORO contract was deployed at.
// A contract either has bytecode at a block or it doesn't — this pinpoints
// the deployment block precisely, so we never waste time scanning blocks
// before the contract existed (which can only ever return zero Transfer logs).
async function findContractDeployBlock(provider) {
  ...
  while (lo <= hi) {
    ...
    const code = await provider.getCode(ORO_TOKEN_ADDRESS, mid);
    if (code && code !== '0x') {
      deployBlock = mid;
      hi = mid - 1;
    } else {
...
async function getStartBlock(provider) {
  ...
  // First-ever run: find the exact block the contract was deployed at.
  const block = await findContractDeployBlock(provider);
  return block;
}
...
async function fetchWindow(provider, fromBlock, toBlock) {
  try {
    ...
    // RPC rejected this range (too large, timed out, or a transient error) — split and retry once.
