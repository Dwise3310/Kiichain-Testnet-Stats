...
const TIME_BUDGET_MS = 50000;      // safe under the 60s Hobby-plan hard cap, with buffer for cleanup/response
...
// Uses eth_getBlock (header/timestamp only) — safe on pruned/non-archive nodes,
// unlike eth_getCode or eth_call at historical heights, which need full state.
...
async function getStartBlock(provider) {
  ...
  // First-ever run: find the block nearest the testnet start date — this uses
  // block headers/timestamps only, which works fine even on a pruned/non-archive node.
